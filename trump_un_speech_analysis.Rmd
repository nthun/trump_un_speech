---
title: "Is this the end of the world or what? - Text mining Donald Trump's UN covfefe"
output:
  html_document:
    df_print: paged
---
...
sentence_sentiment %>% 
        ...
        mutate(roll_sentiment = zoo::rollmean(sum_sentiment, 3, na.pad=TRUE, align="center")) %>% 
        mutate(sentiment_type = case_when(roll_sentiment >0 ~ "positive",
                                          roll_sentiment <0 ~ "negative",
                                          roll_sentiment == 0 ~ "neutral") %>% 
                       fct_rev()
        ) %>% 
        ggplot() +
                aes(x = sentence_num, 
