...
First, let's see which countries were mentioned and how many times. Obviously, the (United States of) America is first! Venezuela, North Korea are on the second and third positions.
...
trump_countries %>% 
        ...
        ggplot() +
                ...
                coord_flip() +
                theme_minimal() +
                labs(title = "Mentions of a country", y = "Mentions", x = NULL)
...
sentence_sentiment %>% 
        ...
        ggplot() +
                aes(x = sentence_num, y = sum_sentiment, label = word %>% str_to_title()) +
                geom_hline(yintercept = 0, color = "grey", linetype = "dashed", size = 1.2) +
                # geom_line() +
                geom_label_repel(aes(fill = sentiment_type), alpha = .8, segment.alpha = 0) +
                geom_smooth(span = 0.1, se = T, size = 1.2, color = "black") +
                theme_minimal() +
                labs(title = "The summarised sentiment of sentences, and the appearance of country names in the speech \nby sentiment in sentence order", x = "Sentence number", y = "Summarised sentiment", subtitle = "The dashed line signifies no sentiment in the sentence. \nCountry label colors show the direction of the sentiment (positive/negative)")
